<template>
    <view class="progress " :class="'progress--'+ptype">
        <!-- 条形进度条 -->
        <view class="progress-bar" v-if="ptype==='line'">
            <view class="progress-bar__outer" :style="{height:strokeHeight+'px'}">
                <view class="progress-bar__inner" :style="barStyle">
                    <!-- 进度条内显示百分比 -->
                    <view v-if="textInside" class="progress__text" style="color:white;"> {{percentage}}% </view>
                </view>
            </view>
           
        </view>
        <!-- 环形进度条 采用SVG实现 -->
        <view class="progress_circle" :style="{width:cwidth+'px',height:cwidth+'px'}" v-else>
            <svg viewBox="0 0 100 100" :style="{width:cwidth+'px',height:cwidth+'px'}">
                <!-- 背景圆形 -->
                <path :d="trackPath" fill="none" :stroke-width="relativeStrokeHeight" stroke="#ffffff" /> 
                <!-- 进度圆形 -->
                <path :d="trackPath" fill="none" :stroke-width="relativeStrokeHeight" :stroke="stroke" :style="circlePathStyle" stroke-linecap="round" />
            </svg>
        </view>
        <view v-if="!textInside" class="progress__text" :style="{fontSize:progressTextSize+'px'}">
